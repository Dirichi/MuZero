from config import MuZeroConfig, make_probabilistic_cartpole_config
from networks.shared_storage import SharedStorage
from self_play.self_play import run_selfplay, run_eval
from training.replay_buffer import ReplayBuffer
from training.probabilistic_training import train_network
...
def muzero(config: MuZeroConfig):
    ...
    for loop in range(config.nb_training_loop):
        ...
        print("Eval score:", run_eval(config, storage, config.nb_episodes))
...
if __name__ == '__main__':
    config = make_probabilistic_cartpole_config()
    muzero(config)
